...
interface NoteBlock {
  _key: string;
  _type: "block";
  children: NoteBlockChild[];
}

interface NoteBlockChild {
  _key: string;
  _type: string;
  marks: string[];
  text: string;
...
const getSanity: () => Promise<Note[]> = async () => {
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
  return ((await sanityClient.fetch(`*[_type == "note"]`)) as Note[]).sort(
    (note1, note2) => {
      return note1.order.localeCompare(note2.order);
    }
  );
};

const getCourses = publicProcedure.query(async () => {
  return [...new Set((await getSanity()).map((n) => n.course))];
});

const getIds = publicProcedure.query(async () => {
  return (await getSanity()).map((n) => n._id);
});
...
const getNote = publicProcedure
  ...
  .query(async ({ input }) => {
    const note = (await getSanity()).find((n) => n._id === input.id);
